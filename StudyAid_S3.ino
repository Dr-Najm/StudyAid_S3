/*
 * StudyAid Firmware v8.4
 * Hardware: M5StickS3 + M5 Unit NFC (ST25R3916, I2C via Grove Port A)
 *
 * Button mapping:
 *   BtnA (front) : Kitar menu / parameter / bangun dari tidur
 *   BtnB (side)  : Sahkan / Pilih / tukar nilai
 *
 * Wiring:
 *   Unit NFC -> Grove Port A (HY2.0-4P) on M5StickS3
 *   (SDA/SCL assigned automatically via M5.getPin)
 *
 * WiFi: SSID=StudyAid  Password=studyaid123  IP=192.168.4.1
 *
 * Libraries required:
 ...
 *   - ESPAsyncWebServer  (web dashboard)
...
const char* subjects[NUM_SUBJECTS] = {
  "Matematik", "Fizik", "Kimia", "Biologi", "Bahasa Inggeris"
};
...
void renderHome() {
  ...
  M5.Display.setCursor(4,6);
  if (sessionActive)
    M5.Display.printf("StudyAid v8 | %s",stateNames[currentState]);
  else
    M5.Display.print("StudyAid v8");
  ...
  if (sessionActive) {
    ...
    M5.Display.setTextColor(WHITE,BLACK);
    const char* opts[]={"Mula Sesi","Daftar Tag","Kalibrasi","Tetapan"};
    for (int i=0;i<4;i++) {
...
void handleBtnA() {
  ...
  switch(currentScreen) {
    case SCREEN_HOME:
      if (!sessionActive) homeMenuIdx=(homeMenuIdx+1)%4;
      break;
    ...
    case SCREEN_SETTINGS:
      settingsParamIdx=(settingsParamIdx+1)%10; break;
  ...
  switch(currentScreen) {
    case SCREEN_HOME:
      if (sessionActive) endSession();
        ...
        switch(homeMenuIdx) {
          case 0: currentScreen=SCREEN_START_SESSION; subjectSelectIdx=0;  break;
          case 1: currentScreen=SCREEN_REGISTER_TAG;  registerSelectIdx=0; break;
          case 2: currentScreen=SCREEN_CALIBRATE;     break;
          case 3: currentScreen=SCREEN_SETTINGS; settingsParamIdx=0; break;
        }
        homeMenuIdx=0;
      }
      break;
...
void setup() {
  ...
  M5.begin(cfg);
  Serial.begin(115200);
  Serial.println("[BOOT] StudyAid v8.4 starting...");
  ...
  M5.Display.setTextSize(1); M5.Display.setCursor(70,58); M5.Display.println("v8.4");
  delay(1000);
  ...
  // WiFi AP
  M5.Display.setCursor(0,80); M5.Display.println("Memulakan WiFi...");
