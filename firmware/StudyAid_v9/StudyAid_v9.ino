...
void endSession() {
  ...
  if (companionReady) {
    ...
    upload["subject_id"] = currentSubject + 1;   // DB is 1-indexed
...
void renderSettings() {
  ...
  if (settingsParamIdx<7) {
    ...
    // v9: hint for companion mode toggle
    M5.Display.setTextColor(CYAN,BLACK); M5.Display.setCursor(0,111);
...
void handleBtnA() {
  ...
  switch(currentScreen) {
    ...
    case SCREEN_SETTINGS:
      settingsParamIdx=(settingsParamIdx+1)%11; break;
...
void setup() {
  ...
  M5.begin(cfg);
  Serial.begin(115200);
  Serial.println("[BOOT] StudyAid v9.0 starting...");
  ...
  M5.Display.setTextSize(1); M5.Display.setCursor(70,58); M5.Display.println("v9.0");
  delay(1000);
